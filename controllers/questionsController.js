...
const question_modes_method = async (req, res, next) => {
    ...
    const user = await User.findById({ _id: req.query.userId }).populate('perks.id').populate('season_results.results').populate('rank')
    let searchName = ''
    const payload = {
    ...
    const updatedUser = await User.findByIdAndUpdate({ _id: req.query.userId }, user, { new: true }).populate('perks.id').populate('season_results.results').populate('rank')
...
const challenges_method = async (req, res, next) => {
    ...
    const user = await User.findById({ _id: req.query.userId }).populate('challenges').populate('perks.id').populate('season_results.results').populate('rank')
    let searchName = ''
    const payload = {
        id: req.query.search,
        lastPlayedDate: moment(new Date).format('YYYY-MM-DD'),
